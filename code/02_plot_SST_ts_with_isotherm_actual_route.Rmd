...
actual_ship <- read.csv('../data/ship_in_situ_info.csv', sep=',') %>% # fyi, had to open in text-edit and hit return to add blank line in next row
  mutate(date = lubridate::ymd(date),
         ...
         lon_deg = ifelse(make180(lon) < 0, str_c(round(make180(lon),2), '°W'),str_c(round(lon,2), '°E')) 
         )
...
p_insitu_sst <- 
  ...
  str_c('Date: ', date,
        '\nLat: ', round(lat, 2), " N",
        '</br>Lon: ', ifelse(make180(lon) < 0, str_c(round(make180(lon),2), ' W'), str_c(round(lon,2), ' E')), 
        '</br>SST: ', insitu_sst, " C"))) +
