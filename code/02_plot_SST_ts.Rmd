...
lons_idx = sapply(1:length(params$deploy_lons), function(x) {findInterval(make360(params$deploy_lons)[x], floor(make360(ship_route_pts$lon)), rightmost.closed  = TRUE)})
...
# plot 
p_sst_ts <- plot_timeseries(data = df, cpal = cpal, ylimits = c(6,20), ybreaks = seq(6,20, by = 2)) +
...
sst_plot <- p_sst_ts +
  labs(
    title = str_c("Average", str_to_title(params$interval), str_to_upper(params$eov), "along shipping route", sep = " "),
    subtitle = str_c(str_to_upper(params$eov), "colors correspond to color locations on map", sep = " "),
    caption = "Source: NOAA Coral Reef Watch, 5-km Daily SST",
...
fig_top <- get_npac_map(xy, cpal) %>% addScaleBar(position = "bottomleft", options = scaleBarOptions())
fig_bottom <- sst_plot
